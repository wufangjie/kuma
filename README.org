...
1. Search the web
2. Run applications (or activate application window)
...
2. psutil (my xubuntu is already installed, though not needed now), windows is needed to get application name
3. For Linux user, python3-xlib is required, which may be already installed by default
4. For Windows user, win32api is required for a faster response\\
...
* startup
** Linux
TODO: I have trouble to make linux working as windows do, it may take a long time
...
** Windows
Run travel_for_windows.pyw and you can call kuma every time you press your global HOTKEY, which defined in travel_for_windows.pyw, default is <Control-;>
...
Maybe the best way is double click file
...
1. Colmuns must be [Keyword, Type, Description, Pattern, Platform, Command]
2. "Keyword" must be unique in certain platform
3. "Type" must in {Web, App, Py}
4. "Description" do not remove it even if it is empty
5. "Pattern" better pattern to match window's appname or title than keyword
